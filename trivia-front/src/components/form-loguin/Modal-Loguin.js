...
export function ModalLoguin({ showLoguin, handleCloseLoguin }) {
 ...
 function SetFieldLoguin(field, value) {
     ...
     if(!!errorsLoguin[field]){  //los operadores !!convierten un valor en booleano
        setErrorsLoguin({ 
         ...errorsLoguin,
         [field]:null})
     }
 ...
 function validateFormLoguin() {
  ...
  if(!emailRegex.test(controlEmail) ||controlEmail === ''){
      newErrors.controlEmail = 'por favor completa este campo correctamente: name@example.com'
  }
